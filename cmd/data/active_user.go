...
func LogoutUser() {
	var tmpUser = model.User{Username: "not-logged-in"}
...
func LoginUser(newActiveUser model.User) {
	...
	if currentUser.IsLoggedIn {
		fmt.Println("already logged in as: ", currentUser.User.Username)
		return
	}

	var isAUser = false
	for _, user := range users {
		if user.Username == newActiveUser.Username {
			isAUser = true
		}
	}
	...
	fmt.Println("loging in as ", newActiveUser.Username)
	var tmpUser = model.User{Username: newActiveUser.Username}
